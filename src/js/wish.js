...
caret.style.cssText = inputText.value;
caret.style.fontFamily = '"Arial Rounded MT Bold","Helvetica Rounded",Arial,sans-serif';
...
function reposition() {
  field = inputText.getBoundingClientRect();
}
...
function burst(intensity) {
  ...
  var rangeMax = Math.min(field.right, offset + 40);
...
simulate(
  '2d', {
    ...
    action: function(x, y) {
      ...
      caret.textContent = inputText.value;

      burst.call(this, 10);

      inputText.classList.add('keyup');
      setTimeout(function() {
        inputText.classList.remove('keyup')
      }, 100);
...
function wish(){
  ...
  document.querySelector("button").onclick = function(){
    ...
    if(inputElement.value != ""){
      ...
      xmlhttp.open("POST", "/rest/wish/", true);
