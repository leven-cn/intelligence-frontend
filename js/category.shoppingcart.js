// 购物车增加/删除事件处理函数
function shoppingcartAddRemoveEventHandler(shoppingList){
    for(var i=0; i<shoppingList.length; i++){
        e = shoppingList[i].getElememtsByTagName("em");
        var count = e[1];
        ...
        e[2].onclick = function(){
            var shoppingItem = this.parentElement.parentElement.dataset;
            var category = {
                "name": shoppingItem.name,
                "unit_price": shoppingItem.unitPrice,
                "unit": shoppingItem.unit,
                "count": shoppingItem.count
            };
            total_fee += parseInt(category.unit_price);
            sessionStorage.setItem(name, JSON.stringify(category));
        }
    }
}
...
// var sloganP = slogan.getElementsByTagName("p");


// if(localStorage.getItem("sloganname") == null){
//   localStorage.setItem("sloganname", "1");
//   sloganShow();
// }else{
